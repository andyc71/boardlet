...
extension View {
    ...
    func selectPhotos(pageLayoutState: PageLayoutState, preselectItems: Bool = false, isAdditive: Bool = false, currentTheme: SharedUITheme) {
        ...
        if root != nil {
            ...
            ZLConfigure(maxSelections: AppSettings.maxSelectionsInPhotoPicker, currentTheme: currentTheme)
    ...
    func selectTopicPhoto(currentTheme: SharedUITheme, onSelect: @escaping (UIImage)->() ) {
        ...
        if root != nil {
            ...
            ZLConfigure(maxSelections: 1, currentTheme: currentTheme)
            ...
            ac.selectImageBlock = { (images, assets, isOriginal) in
                ...
                DispatchQueue.global().async {
                    ...
                    DispatchQueue.main.async {
                        if let image = photoItems.first?.image {
                            onSelect(image)
                            //pageLayoutState.topic.topicImage = image
                        }
                    }
    ...
    func ZLConfigure(maxSelections: Int, currentTheme: SharedUITheme) {
