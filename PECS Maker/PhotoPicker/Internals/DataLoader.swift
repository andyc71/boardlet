...
struct DataLoader
{
    ...
    static let image = DataLoader { $0.imagePublisher.map { _PhotoPickerData.image($0,nil) } }
    static let video = DataLoader { $0.videoPublisher.map { _PhotoPickerData.video($0,nil) } }
    static let livePhoto = DataLoader { $0.livePhotoPublisher.map { _PhotoPickerData.livePhoto($0,nil) } }
}
